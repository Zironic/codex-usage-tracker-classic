"""Decision metadata for every local dashboard API route."""

from __future__ import annotations

from codex_usage_tracker.server.agent_route_inventory import AGENT_ROUTE_PROFILES
from codex_usage_tracker.server.route_profile import DashboardRouteProfile
from codex_usage_tracker.server.route_profile import profile as _profile
from codex_usage_tracker.server.v2_route_inventory import HTTP_V2_ROUTE_PROFILES

_PERSISTED_SNAPSHOT_ROUTES = (
    "overview",
    "tool-output",
    "commands",
    "git-interactions",
    "file-reads",
    "file-modifications",
    "read-productivity",
    "concentration",
    "guided-summary",
)

_PERSISTED_SNAPSHOT_PROFILES = tuple(
    _profile(
        "GET",
        f"/api/diagnostics/{name}",
        f"_handle_diagnostics_{name.replace('-', '_')}",
        "server.diagnostic_snapshots",
        "interactive",
        "Reads one previously computed diagnostic snapshot.",
        "One fixed snapshot payload.",
        "Persisted in SQLite and reused until explicitly refreshed.",
    )
    for name in _PERSISTED_SNAPSHOT_ROUTES
)

_SNAPSHOT_REFRESH_PROFILES = tuple(
    _profile(
        "POST",
        f"/api/diagnostics/{name}/refresh",
        f"_handle_diagnostics_{name.replace('-', '_')}_refresh",
        "server.diagnostic_snapshots",
        "heavy_analysis",
        "Recomputes the named diagnostic over the requested history scope.",
        "One persisted snapshot payload.",
        "Background worker persists the snapshot in SQLite.",
        may_scan_all_history=True,
        execution="async_start",
    )
    for name in _PERSISTED_SNAPSHOT_ROUTES
)


DASHBOARD_ROUTE_PROFILES: tuple[DashboardRouteProfile, ...] = (
    _profile(
        "GET",
        "/api/context",
        "_handle_context",
        "server.context",
        "interactive",
        "Reads context for one explicitly selected usage record.",
        "Configured character window.",
        "On-demand source read; not cached.",
    ),
    _profile(
        "GET",
        "/api/context-settings",
        "_handle_context_settings",
        "server.context_settings",
        "interactive",
        "Reads in-memory server settings.",
        "One compact settings payload.",
        "In-memory state for the server lifetime.",
    ),
    _profile(
        "GET",
        "/api/open-investigator",
        "_handle_open_investigator",
        "server.open_investigator",
        "interactive",
        "Targets one investigator URL.",
        "One action payload.",
        "No persisted result.",
    ),
    _profile(
        "GET",
        "/api/health",
        "_handle_health",
        "server.health",
        "interactive",
        "Reads in-memory process health.",
        "One constant-size health payload.",
        "No persisted state.",
    ),
    _profile(
        "GET",
        "/api/status",
        "_handle_status",
        "server.status",
        "interactive",
        "Reads aggregate refresh metadata.",
        "One compact status payload.",
        "Backed by persisted SQLite metadata.",
    ),
    _profile(
        "GET",
        "/api/readiness",
        "_handle_readiness",
        "server.status",
        "interactive",
        "Checks local MCP configuration and runtime readiness without usage queries.",
        "One constant-size readiness payload.",
        "Computed from local configuration after dashboard hydration.",
    ),
    _profile(
        "GET",
        "/api/diagnostics/dedupe",
        "_handle_dedupe_diagnostics",
        "server.dedupe",
        "bounded_report",
        "Aggregates canonical and physical usage rows, then lists bounded duplicate provenance.",
        "Default 100 physical duplicate rows.",
        "Live SQLite query over indexed dedupe fields; no response cache.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/calls",
        "_handle_calls",
        "server.call_lists",
        "bounded_report",
        "Queries a page; derived filters can require a full matching-row pass.",
        "Default 100 rows with explicit pagination.",
        "Live SQLite query; no response cache.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/call",
        "_handle_call",
        "server.call_detail",
        "interactive",
        "Reads one selected call and bounded adjacent context.",
        "One call detail payload.",
        "Live SQLite query; no response cache.",
    ),
    _profile(
        "GET",
        "/api/threads",
        "_handle_threads",
        "server.threads",
        "bounded_report",
        "Queries aggregate thread rows over the selected history scope.",
        "Default 100 rows with pagination.",
        "Live SQLite aggregate query.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/thread-calls",
        "_handle_thread_calls",
        "server.call_lists",
        "bounded_report",
        "Queries one thread; derived filters can load the full thread.",
        "Default 100 rows with pagination.",
        "Live SQLite query; no response cache.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/summary",
        "_handle_summary",
        "server.summary",
        "bounded_report",
        "Aggregates all matching rows into bounded groups.",
        "Default 20 groups.",
        "Live SQLite/report query; no response cache.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/recommendations",
        "_handle_recommendations",
        "server.recommendations",
        "bounded_report",
        "Ranks persisted recommendation facts before bounded hydration.",
        "Default 20 ranked rows.",
        "Source-generation keyed facts persisted in SQLite.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/allowance/history",
        "_handle_allowance_history",
        "server.allowance",
        "bounded_report",
        "Normalizes allowance observations over the selected scope.",
        "Default 1,000 observations.",
        "Generation/config-keyed large-payload cache over live SQLite and local config.",
        may_scan_all_history=True,
    ),
    *HTTP_V2_ROUTE_PROFILES,
    *AGENT_ROUTE_PROFILES,
    _profile(
        "GET",
        "/api/allowance/diagnostics",
        "_handle_allowance_diagnostics",
        "server.allowance",
        "bounded_report",
        "Evaluates at most the configured observation window.",
        "Default 10,000 observations.",
        "Generation/config-keyed large-payload cache over live SQLite and local config.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/allowance/export",
        "_handle_allowance_export",
        "server.allowance",
        "bounded_report",
        "Builds a strict local allowance evidence export.",
        "Schema-bounded aggregate export.",
        "Computed from SQLite and local config.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/allowance/status",
        "_handle_allowance_status_v2",
        "server.allowance_v2",
        "interactive",
        "Reads the latest canonical allowance state and revision.",
        "One constant-size status payload.",
        "Indexed derived-state reads; never response-cached.",
    ),
    _profile(
        "GET",
        "/api/allowance/series",
        "_handle_allowance_series_v2",
        "server.allowance_v2",
        "bounded_report",
        "Reads canonical cycle points for one finite range.",
        "A supported preset or at most 366 custom days.",
        "Live indexed SQLite query scoped by cycle time.",
    ),
    _profile(
        "GET",
        "/api/allowance/evidence",
        "_handle_allowance_evidence_v2",
        "server.allowance_v2",
        "bounded_report",
        "Reads one revision-bound page of canonical allowance evidence.",
        "At most 500 evidence transitions.",
        "Indexed keyset pagination; stale revisions return a conflict.",
    ),
    _profile(
        "GET",
        "/api/allowance/analysis",
        "_handle_allowance_analysis_v2",
        "server.allowance_v2",
        "interactive",
        "Reads one compatible persisted allowance analysis snapshot.",
        "One fixed snapshot or missing-state payload.",
        "Exact semantic-key lookup in SQLite; never computes analysis.",
    ),
    _profile(
        "POST",
        "/api/allowance/analysis/jobs",
        "_handle_allowance_analysis_job_start_v2",
        "server.allowance_v2",
        "heavy_analysis",
        "Starts or reuses one cycle-block allowance analysis.",
        "One in-process job handle; completed result persists in SQLite.",
        "Concurrent work deduplicated by semantic analysis key.",
        execution="async_start",
    ),
    _profile(
        "GET",
        "/api/allowance/analysis/jobs",
        "_handle_allowance_analysis_job_status_v2",
        "server.allowance_v2",
        "interactive",
        "Reads progress for one in-process allowance analysis job.",
        "One compact job-status payload.",
        "In-memory status; completed analysis remains persisted separately.",
        execution="poll",
    ),
    _profile(
        "GET",
        "/api/reports/pack",
        "_handle_reports_pack",
        "server.reports",
        "bounded_report",
        "Builds bounded report sections; derived filters may scan history.",
        "Default 100 rows and 8 evidence rows.",
        "Computed per request; no response cache.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/investigations/agentic",
        "_handle_investigation_agentic",
        "server.investigations",
        "bounded_report",
        "Composes bounded indexed diagnostic reports over matching history.",
        "Default 5 evidence rows.",
        "Generation-keyed in-memory response cache; recomputed after index or config changes.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/investigations/repeated-files",
        "_handle_investigation_repeated_file_rediscovery",
        "server.investigations",
        "bounded_report",
        "Ranks persisted file-event facts over matching history.",
        "Default 20 patterns and 3 samples.",
        "Generation-keyed in-memory response cache.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/investigations/shell-churn",
        "_handle_investigation_shell_churn",
        "server.investigations",
        "bounded_report",
        "Ranks persisted command facts over matching history.",
        "Default 20 patterns and 3 samples.",
        "Generation-keyed in-memory response cache.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/investigations/large-low-output",
        "_handle_investigation_large_low_output",
        "server.investigations",
        "bounded_report",
        "Ranks aggregate call rows with indexed token predicates.",
        "Default 20 rows.",
        "Generation-keyed in-memory response cache.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/investigations/walk",
        "_handle_investigation_walk",
        "server.investigations",
        "bounded_report",
        "Composes bounded indexed hypotheses over matching history.",
        "Default 5 evidence rows.",
        "Generation-keyed in-memory response cache.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/diagnostics/summary",
        "_handle_diagnostics_summary",
        "server.diagnostic_facts",
        "bounded_report",
        "Aggregates persisted diagnostic facts over matching scope.",
        "Default 20 groups.",
        "Live SQLite query over persisted facts.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/diagnostics/facts",
        "_handle_diagnostics_facts",
        "server.diagnostic_facts",
        "bounded_report",
        "Queries persisted diagnostic facts with filters.",
        "Default 50 facts.",
        "Generation-keyed response cache over indexed persisted facts.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/diagnostics/fact-calls",
        "_handle_diagnostics_fact_calls",
        "server.diagnostic_facts",
        "bounded_report",
        "Resolves supporting calls for request-bounded diagnostic facts.",
        "Request-bounded evidence rows.",
        "Live SQLite query; no response cache.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/diagnostics/compactions",
        "_handle_diagnostics_facts",
        "server.diagnostic_facts",
        "bounded_report",
        "Queries persisted compaction facts.",
        "Default 50 facts.",
        "Generation-keyed response cache over indexed persisted facts.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/diagnostics/tools",
        "_handle_diagnostics_facts",
        "server.diagnostic_facts",
        "bounded_report",
        "Queries persisted tool facts.",
        "Default 50 facts.",
        "Generation-keyed response cache over indexed persisted facts.",
        may_scan_all_history=True,
    ),
    *_PERSISTED_SNAPSHOT_PROFILES,
    _profile(
        "GET",
        "/api/diagnostics/usage-drain",
        "_handle_diagnostics_usage_drain",
        "server.diagnostic_snapshots",
        "interactive",
        "Reads the persisted usage-drain snapshot.",
        "One fixed snapshot payload.",
        "Persisted in SQLite and reused until explicitly refreshed.",
    ),
    _profile(
        "GET",
        "/api/compression/status",
        "_handle_compression_status",
        "server.compression_routes",
        "interactive",
        "Reads one persistent Compression Lab run status by ID.",
        "One compact status payload capped by the shared 4 KiB contract.",
        "Read-only SQLite run lookup; does not start or block worker persistence.",
        execution="poll",
    ),
    _profile(
        "GET",
        "/api/compression/profile",
        "_handle_compression_profile",
        "server.compression_routes",
        "bounded_report",
        "Reads one exact completed run or newest valid profile for the requested scope.",
        "One compact profile payload capped by the shared 8 KiB contract.",
        "Persisted in SQLite and shared with MCP consumers.",
    ),
    _profile(
        "POST",
        "/api/compression/start",
        "_handle_compression_start",
        "server.compression_routes",
        "heavy_analysis",
        "Starts or reuses an asynchronous Compression Lab run for the requested scope.",
        "One compact job handle.",
        "Persistent completed-run cache plus process-owned background worker.",
        may_scan_all_history=True,
        execution="async_start",
    ),
    _profile(
        "GET",
        "/api/usage",
        "_handle_usage",
        "server.usage_refresh",
        "bounded_report",
        "Reads a configured window; refresh=1 may rebuild all source history.",
        "Configured dashboard row limit or explicit all-history request.",
        "Reads SQLite; optional refresh updates the persisted index.",
        may_scan_all_history=True,
    ),
    _profile(
        "GET",
        "/api/refresh/start",
        "_handle_refresh_start",
        "server.usage_refresh",
        "heavy_analysis",
        "Starts an asynchronous source-history refresh.",
        "One job handle.",
        "Persists index changes; job state is in process.",
        may_scan_all_history=True,
        execution="async_start",
    ),
    _profile(
        "GET",
        "/api/refresh/status",
        "_handle_refresh_status",
        "server.usage_refresh",
        "interactive",
        "Reads one asynchronous refresh job.",
        "One compact job status.",
        "In-process registry for the server lifetime.",
        execution="poll",
    ),
    _profile(
        "GET",
        "/api/diagnostics/refresh/status",
        "_handle_diagnostics_refresh_status",
        "server.diagnostic_jobs",
        "interactive",
        "Reads one diagnostic refresh job without touching persisted snapshots.",
        "One compact job status.",
        "Read-only in-process registry lookup.",
        execution="poll",
    ),
    _profile(
        "POST",
        "/api/diagnostics/refresh",
        "_handle_diagnostics_refresh",
        "server.diagnostic_snapshots",
        "heavy_analysis",
        "Recomputes every diagnostic snapshot over the selected history.",
        "One asynchronous job handle.",
        "Background worker serializes and persists snapshots.",
        may_scan_all_history=True,
        execution="async_start",
    ),
    *_SNAPSHOT_REFRESH_PROFILES,
    _profile(
        "POST",
        "/api/diagnostics/usage-drain/refresh",
        "_handle_diagnostics_usage_drain_refresh",
        "server.diagnostic_snapshots",
        "heavy_analysis",
        "Recomputes the usage-drain snapshot over history.",
        "One persisted snapshot payload.",
        "Background worker persists the snapshot in SQLite.",
        may_scan_all_history=True,
        execution="async_start",
    ),
)
